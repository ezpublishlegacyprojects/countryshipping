{let shipping_values=$class_attribute.content
     ...
     first_value_id=0
     add_value_id=0
     default_shipping_group=fetch('ezshipping', 'shipping_group', hash('shippinggroup_id', $class_attribute.data_int1))}
<div class="block">
  <div class="block">
    <div class="element">
    <label>{"Default shipping group"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping')}</label>
    {if is_object($default_shipping_group)}
    {$default_shipping_group.name|wash(xhtml)}
    {else}
    {"Custom"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping')}
    {/if}
    </div>
    <div class="element">
      <label>{"Default currency code"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping')}</label>
      {$preferred_currency_code|wash(xhtml)}
    </div>
    <div class="break"></div>
  </div>
  <div class="block">
      <label>{"Default shipping:"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping')}</label>
      <table class="list" border="0">
      <tr>
      <th width="9%">{"Currency"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping')}</th>
      <th width="45%" colspan="2">{"Value first"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping')}</th>
      <th width="25%" colspan="2">{"Value additional"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping')}</th>
      <th width="20%">{"Auto rate"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping')}</th>
      </tr>
      ...
      {if is_set($existing_currencies[$code])}
      ...
      <td>{$currency_object.rate_value|wash(xhtml)}</td>
      </tr>
      {/let}
      {/foreach}
      </table>
  ...
  <div class="block">
      <label>{"Additional country shipping (first / additional):"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping')}</label>
      <table class="list" border="0">
      <tr>
      <th width="15%">{"Country"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping')}</th>
      {foreach $currency_list as $code => $currency_object}
      <th>{"Values (%code)"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping', '', hash('%code', $code|wash(xhtml)))}</th>
      {/foreach}
      </tr>
      <tr class="bglight">
      <td>{"Other countries"|i18n('countryshipping/design/standard/class/datatype/view/ezshipping')}</td>
      ...
      {if is_set($additional_country_default[$code][$pricegroup_hash['additional_value']]['auto_value'])}
         {set add_value=concat($additional_country_default[$code][$pricegroup_hash['additional_value']]['auto_value'], ' (Auto)'|i18n('countryshipping/design/standard/class/datatype/view/ezshipping'))}
      {/if}
      <td>{$currency_object.symbol|wash(xhtml)} &nbsp;{$first_value} / {$add_value}</td>
      {/let}
      {/foreach}
      </tr>
    {foreach $additional_country_list as $country_name => $additional_country
      ...
      {if is_set($additional_country[$code][$pricegroup_hash['additional_value']]['auto_value'])}
         {set add_value=concat($additional_country[$code][$pricegroup_hash['additional_value']]['auto_value'], ' (Auto)'|i18n('countryshipping/design/standard/class/datatype/view/ezshipping'))}
      {/if}
